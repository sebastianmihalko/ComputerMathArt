public class RandomFunction
{
  ...
  public static void draw(int c)
  {
    ...
    for(int i = 0; i <= width; i++)
    {
      for(int j = 0; j <= height; j++)
      {
        ...
        Draw.setColor(0,colorvalue,colorvalue);
  ...
  public static Function randomFunction(int complexity)
  {
    ...
    if(complexity == 0)
    {
      if(random < 0.4) return new X();
      else if (random < 0.6) return new Random();
      else return new Y();
    }
    else
    {
      if(random < 0.2) return new Average(randomFunction(complexity-1),randomFunction(complexity-1));
      else if(random < 0.4) return new Product(randomFunction(complexity-1),randomFunction(complexity-1));
      else if(random < 0.6) return new Sine(randomFunction(complexity-1));
      else if(random < 0.8) return new Floor(randomFunction(complexity-1));
